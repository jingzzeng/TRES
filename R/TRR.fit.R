...
TRR.fit <- function(x, y, u, method=c('standard', 'FG', '1D', 'ECD', 'PLS'), Gamma_init = NULL) {
  ...
  output <- list(x = x_old, y = y_old, call = cl, method = method, coefficients=Bhat, Gamma=Gamma, Sigma=Sig, fitted.values = fitted.values, residuals = residuals, M_list = M_list, U_list = U_list, W0_list = W0_list, test_out_list = test_out_list, En = En, S_list_1 = res$S_list_1, S_list_2 = res$S_list_2)
  class(output) <- "Tenv"
  output
}
